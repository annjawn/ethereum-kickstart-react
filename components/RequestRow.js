...
class RequestRow extends Component{
  ...
  render() {
    ...
    const readyToFinalize = this.props.request.approvalCount > this.props.totalApprovers / 2;
    return (
      <Row disabled={this.props.request.complete} positive={readyToFinalize && !this.props.request.complete}>
        ...
        <Cell>{this.props.request.approvalCount}/{this.props.totalApprovers}</Cell>
        <Cell>
        ...
        <Cell>
        { this.props.request.complete ? null :(
            <Button color="red" basic onClick={this.onFinalize}> Finalize
            </Button>
          )
        }
        </Cell>
